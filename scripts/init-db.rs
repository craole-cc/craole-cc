...
enum InitDbError {
  ...
  #[diagnostic(help("Install sqlx-cli with: cargo install sqlx-cli --no-default-features --features sqlite"))]
...
fn main() -> Result<()> {
  ...
  if db_path.is_file() {
    if cli.force {
      ...
      success("Database already exists. Skipping. Use -f / --force / --reset to reinitialise.");
      return Ok(());
    }
  }
