...
pub fn view() -> impl IntoView {
    ...
    view! {
      <section id="experience" class="mb-20">
        <h2 class="mb-8 text-4xl font-bold text-blue-400">"Experience"</h2>
        <div class="space-y-8">
          {experiences
            .into_iter()
...
fn card(experience: Experience) -> impl IntoView {
    view! {
      <div class="py-2 pl-6 border-l-4 border-blue-500">
        <h3 class="mb-2 text-2xl font-semibold text-slate-100">{experience.title}</h3>
        <div class="mb-4 text-sm font-medium text-blue-400">{experience.period}</div>
        <ul class="space-y-2">
          {experience
            .points
            .into_iter()
