    ...
    <Suspense fallback=move || {
      ...
      {move || {
        project
          ...
          .map(|res| match res {
            Ok(Some(p)) => {
              // Build the list of (icon, label) pairs for tags that have known icons
              ...
              view! {
                <article class="dev-detail readable">
                  ...
                  // -- Tech Stack icons (from tags with known icons)
                  {(!tech_icons.is_empty())
                    .then(|| {
                      view! {
                  ...
                  // -- Screenshots gallery (from DB or README images)
                  {(!p.screenshots.is_empty())
                    .then(|| {
                      view! {
                    ...
                    .map(|html| {
                      view! {
                        <section class="dev-detail__readme">
                          ...
                          <div class="dev-detail__readme-body markdown" inner_html=html.clone() />
                        </section>
                      }
                    })}
